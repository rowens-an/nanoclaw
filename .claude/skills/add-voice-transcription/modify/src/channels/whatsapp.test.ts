...
describe('WhatsAppChannel', () => {
  ...
  describe('connection lifecycle', () => {
    ...
    it('flushes outgoing queue on reconnect', async () => {
      ...
      expect(fakeSocket.sendMessage).toHaveBeenCalledWith(
        'test@g.us',
        { text: 'NanoClaude: Queued message' },
      );
    });
      ...
      await triggerMessages([
        {
          ...
          message: { conversation: 'Hello NanoClaude' },
          pushName: 'Alice',
  ...
  describe('outgoing message queue', () => {
    it('sends message directly when connected', async () => {
      ...
      expect(fakeSocket.sendMessage).toHaveBeenCalledWith('test@g.us', { text: 'NanoClaude: Hello' });
    });
    ...
    it('prefixes direct chat messages on shared number', async () => {
      ...
      expect(fakeSocket.sendMessage).toHaveBeenCalledWith('123@s.whatsapp.net', { text: 'NanoClaude: Hello' });
    });
    ...
    it('flushes multiple queued messages in order', async () => {
      ...
      expect(fakeSocket.sendMessage).toHaveBeenNthCalledWith(1, 'test@g.us', { text: 'NanoClaude: First' });
      expect(fakeSocket.sendMessage).toHaveBeenNthCalledWith(2, 'test@g.us', { text: 'NanoClaude: Second' });
      expect(fakeSocket.sendMessage).toHaveBeenNthCalledWith(3, 'test@g.us', { text: 'NanoClaude: Third' });
    });
  });
