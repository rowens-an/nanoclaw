...
describe('SlackChannel', () => {
  ...
  describe('sendMessage', () => {
    ...
    it('sends with username override when sender is provided', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();

      await channel.sendMessage('slack:C0123456789', 'Found 3 results', 'Researcher');
    ...
    it('does not include username when sender is not provided', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
    ...
    it('preserves sender in queued messages and flushes with username', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);

      // Queue while disconnected
      await channel.sendMessage('slack:C0123456789', 'Queued msg', 'Coder');
