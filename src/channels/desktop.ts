...
export class DesktopChannel implements Channel {
    ...
    return new Promise((resolve) => {
      this.server.on('error', (err: NodeJS.ErrnoException) => {
        if (err.code === 'EADDRINUSE') {
          logger.warn(
            { port: this.port },
            `Desktop channel port ${this.port} in use, retrying in 2s...`,
          );
          setTimeout(() => {
            this.server.close();
          ...
          logger.error({ err }, 'Desktop channel server error');
        }
      });
      this.server.listen(this.port, '127.0.0.1', () => {
        this.connected = true;
  ...
  private handleClearSession(jid: string, res: http.ServerResponse): void {
    if (!jid.startsWith('desktop:')) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: 'Not a desktop group' }));
      return;
    ...
    const deleted = deleteMessagesForChat(jid);
    deleteSession(group.folder);

    logger.info({ jid, messagesDeleted: deleted }, 'Desktop session cleared');
