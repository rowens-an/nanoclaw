...
describe('parameterized SQL registration', () => {
  ...
  it('registers a group with parameterized query', () => {
    ...
    expect(row.trigger_pattern).toBe('@NanoClaude');
  ...
  it('prevents SQL injection in JID field', () => {
    ...
    db.prepare(
      `INSERT OR REPLACE INTO registered_groups
       (jid, name, folder, trigger_pattern, added_at, container_config, requires_trigger)
       VALUES (?, ?, ?, ?, ?, NULL, ?)`,
    ).run(maliciousJid, 'Evil', 'evil', '@NanoClaude', '2024-01-01T00:00:00.000Z', 1);
...
describe('file templating', () => {
  it('replaces assistant name in CLAUDE.md content', () => {
    let content = '# NanoClaude\n\nYou are NanoClaude, a personal assistant.';

    content = content.replace(/^# NanoClaude$/m, '# Nova');
    content = content.replace(/You are NanoClaude/g, 'You are Nova');
  ...
  it('handles names with special regex characters', () => {
    ...
    content = content.replace(/^# NanoClaude$/m, `# ${newName}`);
    content = content.replace(/You are NanoClaude/g, `You are ${newName}`);
