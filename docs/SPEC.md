...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'NanoClaude';
...
Messages must start with the trigger pattern (default: `@NanoClaude`):
- `@NanoClaude what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @NanoClaude` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @NanoClaude what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@NanoClaude what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@NanoClaude add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@NanoClaude remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@NanoClaude list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@NanoClaude remember I prefer dark mode` | Add to global memory |
...
```
User: @NanoClaude remind me every Monday at 9am to review the weekly metrics
...
```
User: @NanoClaude at 5pm today, send me a summary of today's emails
...
From any group:
- `@NanoClaude list my scheduled tasks` - View tasks for this group
- `@NanoClaude pause task [id]` - Pause a task
- `@NanoClaude resume task [id]` - Resume a paused task
- `@NanoClaude cancel task [id]` - Delete a task

From main channel:
- `@NanoClaude list all tasks` - View tasks from all groups
- `@NanoClaude schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@NanoClaude add group "Name"` in main |
